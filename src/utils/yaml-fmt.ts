...
export const applySortFiles = async (globPaths: string, options?: SortOptions) =>{
  const files = glob.sync(globPaths)
  try {
    const promises = files.map(file => {
      const doc: Record<string, any> = yaml.load(fs.readFileSync(file, "utf8"), {json: true}) as Record<string, any>;
